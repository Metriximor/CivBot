...
async def find_discord_invite(my_channel, id):
    ...
    discord_invite = None
    view_invite_perms = (
        my_channel.guild.get_member(id).permissions_in(my_channel).manage_guild
    )
    if view_invite_perms:
        ...
        create_invite_perms = (
            my_channel.guild.get_member(id)
            .permissions_in(my_channel)
...
class VoiceRelayCog(commands.Cog):
    ...
    async def selectvoicechannels(self, ctx):
        ...
        await ctx.channel.send(
            "Will display a list of all voice channels in guild in UI order from top to bottom. Type `confirm` to continue"
        )
        try:
            ...
            if msg.content in ["confirm", "c"]:
                ...
                for x in range(0, len(ctx.guild.voice_channels)):
                    if (
                        ctx.guild.voice_channels[x].category_id
                        and ctx.guild.voice_channels[x].category_id not in category_ids
                    ):
                        output += (
                            "\n  ---"
                ...
                await ctx.channel.send(
                    output
                    + "```"
                    + "Select a channel number or comma seperated list of channel numbers. Prefix a number with `^` to remove an already set channel from relay"
                )
                try:
                    msg = await self.bot.wait_for(
                        "message", timeout=20, check=check_author
                    )
                    ...
                    for x in msg.content.split(","):
                        if (
                            ...
                            and 1
                            <= int(x.replace("^", ""))
                            < len(ctx.guild.voice_channels) + 1
                        ):
                            if "^" in x:
                                channels_relayed = list(
                                    filter(
                                        lambda a: a
    ...
    async def voicerelay(self, ctx, arg):
        ...
        if arg == "create" or arg == "remove":
            ...
            relay_channels = [
                guild["channel_id"]
                for guild in guilds[str(ctx.guild.id)]["view_relay_channels"]
            ]
            if arg == "create" and ctx.channel.id not in relay_channels:
                guilds[str(ctx.guild.id)]["view_relay_channels"].append(
                    {"channel_id": ctx.channel.id}
                )
            elif arg == "remove" and ctx.channel.id in relay_channels:
                for t in range(
                    0, len(guilds[str(ctx.guild.id)]["view_relay_channels"])
                ):
                    if (
                        guilds[str(ctx.guild.id)]["view_relay_channels"][t][
                            "channel_id"
                        ]
                        == ctx.channel.id
                    ):
        ...
        if before.channel != after.channel and guilds_json is not None:
            for guild_id in guilds_json.keys():
                if "channels_relayed" in guilds_json[guild_id].keys():
                    ...
                    if (
                        ...
                        in guilds_json[guild_id]["channels_relayed"]
                    ):
            ...
            for guild_id in guilds_json.keys():
                ...
                if "view_relay_channels" in guilds_json[guild_id].keys():
                    for j in range(
                        0, len(guilds_json[guild_id]["view_relay_channels"])
                    ):
                        try:
                            channel_to_send_in = self.bot.get_channel(
                                int(
                                    guilds_json[guild_id]["view_relay_channels"][j][
                                        "channel_id"
                                    ]
                                )
                            )
                            ...
                            if channel_to_send_in is not None:
                                if (
                                    "message_id"
                                    in guilds_json[guild_id]["view_relay_channels"][
                                        j
                                    ].keys()
                                    and guilds_json[guild_id]["view_relay_channels"][j][
                                        "message_id"
                                    ]
                                    is not None
                                ):
                                    await (
                                        await channel_to_send_in.fetch_message(
                                            guilds_json[guild_id][
                                                "view_relay_channels"
                                            ][j]["message_id"]
                                        )
                                    ).edit(content=VC_relay_message)
                                else:
                                    msg_to_store = await channel_to_send_in.send(
                                        VC_relay_message
                                    )
                                    guilds_json[guild_id]["view_relay_channels"][j][
                                        "message_id"
