...
def draw_pearl_image(pearled_player: str, pearled_by: str, now: str):
    ...
    colors = [
        [(85, 255, 255), (255, 255, 255), (170, 170, 170), (85, 85, 85)],
    ...
    for i in reversed(range(0, 2)):
        ...
        draw.text(
            (156 - offset, 158 - offset), pearled_by, font=font, fill=colors[i][2]
        )
...
def draw_joinedweezer_image(players):
    ...
    if len(players) > len(to_send):
        for x in range(0, len(drawn_players)):
            if drawn_players[x] != "":
                query_url = f"https://minotar.net/armor/body/{get_player_data(drawn_players[x]).name}/{int((rect_corners[x][0][1] - rect_corners[x][1][1]) / 2)}.png"
...
def draw_derelict_image(input_string):
    ...
    basewidth = math.floor(
        max([background.height, background.width]) / (2.2 - random_scale_factor * 0.1)
    )
    ...
    background.paste(
        sign,
        (
            background.width - sign.width - random.randrange(0, 30),
            background.height
            - sign.height
            + math.floor(
                sign.height / (10 + random_scale_factor) + random_scale_factor * 10
            ),
        ),
        sign,
    )
...
def draw_dontcare_image(username):
    r = requests.get("https://minotar.net/helm/" + str(username) + "/325.png")
...
def draw_grimreminder_image(player):
    ...
    r = requests.get("https://minotar.net/helm/" + player + "/254.png")
...
def draw_chart_image(chart_data, chart_code):
    ...
    for player_name in chart_data[str(chart_code)]["chart_data"].keys():
        ...
        fixed_coords = [
            ...
            int(
                (background.height - ((coords[1] / 100) * background.height))
                - (face_width / 2)
            ),
        ]
        if get_player_data(player_name) is not None:
            if path.exists("resources/playerheads/" + str(player_name) + ".png"):
                ...
                with open(
                    "resources/playerheads/" + str(player_name) + ".png", "wb"
                ) as f:
                    f.write(r.content)
...
class ImageMeme(commands.Cog):
    ...
    async def animemer(self, ctx):
        ...
        await ctx.channel.send(
            file=discord.File("resources/ImageMeme/Animemer_template.png")
        )
    ...
    async def edit(self, ctx, chart_code, playername, xpos, ypos):
        ...
        if get_player_data(playername) is None:
            ...
            return
        if str(chart_code) in chart_data.keys():
            if str(playername) not in chart_data[chart_code]["chart_data"].keys():
                chart_data[chart_code]["chart_data"][str(playername)] = {}

            chart_data[chart_code]["chart_data"][str(playername)][
                str(ctx.author.id)
            ] = {"x_coord": str(xpos), "y_coord": str(ypos)}
    ...
    async def create(self, ctx, chart_name):
        ...
        with open("resources/ImageMeme/Chart/chart_creator.txt", "w") as json_file:
            json.dump(chart_data, json_file)
        await ctx.send(
            ":white_check_mark: Chart created. Chart can be accessed with the code "
            + str(chart_id)
        )
