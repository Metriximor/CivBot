...
async def get_account_info_from_web(ign):
    ...
    names = [
        ...
        for t in soup1.find_all(class_="text-right")
        if t.span
    ]
    ...
    return {k: locals()[k] for k in "mcstats_url head_url names servers".split()}


def parse_mcstats_name_change_time(s):
    if s == "Name at registration":
        return None
...
def extract_mcstats_servers(soup):
    ...
    return [
        (
            t.find_all(class_="copy-ip")[0]["data-clipboard-text"].replace(
                ":25565", ""
            ),
...
class MiscUtilities(commands.Cog):
    ...
    async def whois(self, ctx, *, content):
        ...
        if info["names"][-1][1]:
            ...
            name_age = "never"
        joined_names = " ".join(
            sorted(set(name for (name, date) in info["names"][:-1]))
        )
        joined_names = joined_names or "None"
        joined_servers = ", ".join(
            "%s (%sh)" % (ip, hours) for (ip, motd, last_seen, hours) in info["servers"]
        )
    ...
    **{ign_clean}**
    ...
    async def whereis(self, ctx, x, z, fromRelay=False):
        ...
        if re.match("[+-]?\d", x) and re.match("[+-]?\d", z):
            ...
            for k in ccmap["features"]:
                distance = int(
                    math.sqrt((int(x) - int(k["x"])) ** 2 + (int(z) - int(k["z"])) ** 2)
                )
            ...
            for d in range(0, 14 if not fromRelay else 4):
                ...
                rad = math.atan2(
                    ccmap["features"][ind]["z"] - int(z),
                    ccmap["features"][ind]["x"] - int(x),
                )
                ...
                ix = round(math.degrees(rad) / (360.0 / len(dirs)))
                xtra = ""
                ...
                out += (
                    str(distances[list(distances)[d]]).rjust(4, " ")
                    + "  blocks "
                    + (dirs[ix % len(dirs)]).rjust(5, " ")
                    + "  "
                    + ccmap["features"][ind]["name"]
                    + xtra
                    + "\n"
                )
            if not fromRelay:
                ...
                await ctx.channel.send(
                    (str(out).replace("```asciidoc", "").replace("::", "✪"))
                )
    ...
    async def getschematic(self, ctx, schematicfile):
        ...
        for x in set(blocks):
            ...
            block_counts.append(
                str(blocks).replace("B; ", "").split(", ").count(str(x).upper())
            )
        ...
        for i in range(0, len(block_ids)):
            ...
            if "-" in block_ids[i]:
                b_name = "???"
            output += (
                b_name.rjust(30, " ") + "  " + str(block_counts[i]).ljust(2, " ") + "\n"
            )
