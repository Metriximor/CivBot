...
def wiardify(text):
    ...
    for i in range(len(text)):
        try:
            if (
                text[i - 1].lower() in vowels
                and text[i + 1].lower() in vowels
...
class TextMeme(commands.Cog):
    ...
    @commands.command()
    async def oracle(self, ctx: Context):
        """Gives a oraclist response"""
        wordlist = [
            "That is definitely not canon and may in fact be illegal",
            "Roleplay Detected",
    ...
    @commands.command()
    async def thraldrek(self, ctx: Context):
    ...
    @commands.command()
    async def topher(self, ctx: Context):
    ...
    @commands.command()
    async def freestyle(self, ctx: Context):
    ...
    async def generateplugin(self, ctx, content="1"):
        ...
        for x in range(0, num):
            ...
            if (random.randrange(10) >= 8 and gen_prefix == "") or random.randrange(
                10
            ) >= 6:
    ...
    @commands.command()
    async def nebulang(self, ctx: Context):
        """Nebulang"""
    ...
    async def pickle(self, ctx, content):
        ...
        if len(adjs) > 0:
            await ctx.channel.send(
                random.choice(adjs).capitalize() + " " + escape_mentions(content)
            )

    @commands.command()
    async def trolling(self, ctx: Context):
    ...
    @commands.command()
    async def pac(self, ctx: Context):
        """PacGaming"""
        wordlist = [
            "Gaming",
            "Blaming",
    ...
    @commands.command()
    async def guy(self, ctx: Context):
    ...
    @commands.command()
    async def allen(self, ctx: Context):
        """Dispenses some allenist thought"""
        wordlist = [
            "we eat of squareblob's flesh and drink of squareblob's blood",
    ...
    @commands.command()
    async def askthemayor(self, ctx: Context):
    ...
    @commands.command()
    async def generatename(self, ctx: Context):
        """Generates a custom minecraft username"""
        await ctx.channel.send(
            "Random Minecraft name generator is creating your username..."
        )
        ...
        await ctx.channel.send(
            "Your username is **minemaster" + str(random.randint(100, 999)) + "**"
        )
    ...
    async def pplocate(self, ctx, *, content):
        ...
        if content.lower() in locations:
            ...
            await ctx.channel.send(
                "**" + escape_mentions(content) + "**'s pearl could not be located"
            )

    @commands.command()
    async def election(self, ctx: commands.Context, *, content: str) -> None:
