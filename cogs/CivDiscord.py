...
class CivDiscord(commands.Cog):
    ...
    async def add(self, ctx, content):
        ...
        if discord_data is not None:
            ...
            if inv_id not in discord_data.keys():
                ...
                discord_data[inv_id]["approximate_member_count"] = str(
    ...
    async def nick(self, ctx, inv_code, name):
        ...
        try:
            ...
            if discord_data is not None:
                ...
                if inv_id not in discord_data.keys():
                    ctx.send(
                        "This invite id must first be submitted. Use %civdiscord add INVITE"
                    )
                else:
                    ...
                    if name in discord_data[inv_id]["nickname"]:
                        ...
                        with open(
                            "resources/CivDiscord/discord_data.json", "w"
                        ) as outfile:
                            json.dump(discord_data, outfile)
    ...
    async def rate(self, ctx, inv_code, rating):
        ...
        try:
            ...
            if discord_data is not None:
                ...
                if inv_id not in discord_data.keys():
                    ...
                    if str(ctx.author.id) in discord_data[inv_id]["rating"].keys():
                        old_rating = int(
                            discord_data[inv_id]["rating"][str(ctx.author.id)]
                        )
                    if rating.isdigit() and 1 <= int(rating) <= 5:
                        ...
                        if old_rating is not None:
                            msg += (
                                ...
                                + (
                                    "".join(
                                        [":star:" for x in range(0, int(old_rating))]
                                    )
                                    + (
                                        "+"
                                        if old_rating > int(old_rating) + 0.4
                                        else ""
                                    )
                                )
                                + "\n"
                        ...
                        msg += (
                            "Rating :"
                            + (
                                "".join([":star:" for x in range(0, int(rating))])
                                + ("+" if rating > int(rating) + 0.4 else "")
                            )
                            + "\n"
                        )
                        with open(
            ...
            await ctx.send(
                'Nickname must be in format "invite_code nickname". Invite code must be valid'
            )
    ...
    async def search(self, ctx, content):
        ...
        for key in discord_data:
            if "current_name" in discord_data[key].keys():
                if len(content) > 4:
                    match = fuzz.token_set_ratio(
                        content, discord_data[key]["current_name"]
                    )
                else:
            ...
            if match > 50:
                if (
                    "valid_invites" in discord_data[key].keys()
                    and len(discord_data[key]["valid_invites"]) > 0
                ):
                    keys.append(key)
        ...
        if len(matches) > 0:
            ...
            for i in range(0, stop):
                ...
                if "rating" in discord_data[keys[i]].keys():
                    for rating_key in discord_data[keys[i]]["rating"].keys():
                        rating += discord_data[keys[i]]["rating"][rating_key]
                ...
                resp += (
                    discord_data[keys[i]]["valid_invites"][0]
